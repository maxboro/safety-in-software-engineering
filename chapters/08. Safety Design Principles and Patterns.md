---
title: "Safety Design Principles and Patterns"
permalink: /chapters/safety-design/
---
# Safety Design Principles and Patterns

![General](assets/safety_design_general.png)  
...
In NIST’s glossary, **fail safe** is a termination mode that prevents damage to specified resources/entities (including life/property), even though a failure might still lead to a security compromise. ( [2])
...
NIST defines **fail secure** as termination that prevents loss of the secure state when a failure occurs or is detected. ( [5])
...
Defense in depth is layered controls so that if one control fails or is bypassed, another still protects the safety objective. NIST defines defense-in-depth as a multi-layer strategy integrating people/technology/operations across multiple layers. ( [6])
...
* **N-modular redundancy** with **majority voting** (e.g., Triple Modular Redundancy: 3 channels + voter, 2-out-of-3 wins). ([Wikipedia][7])
...
* **Fail-safe / fail-secure**: clarify whether you’re protecting life/property vs secure state. ( [2])
...
* **Defense in depth**: layered, partially independent controls. ( [6])
* **Redundancy + diversity + voting**: replicate and compare, but manage common-cause failures. ([Wikipedia][7])
