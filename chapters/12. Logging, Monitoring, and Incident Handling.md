...
* **Tamper resistance / access control** and retention rules—logs are evidence. ([ЦРБК NIST][2])
...
* OSHA defines a near-miss as an event with no damage/injury but where a slight change could have caused harm. ([Управление по охране труда и здоровья][4])
...
Incident handling should be a defined lifecycle, not an improvised meeting. NIST’s incident response model explicitly includes **preparation**, **detection & analysis**, **containment/eradication/recovery**, and **post-incident activity**. ([ЦРБК NIST][7])
